...
class ALL_Controllers
{
    ...
    public function setHargaObatFrom($list_id_jadwal_konsultasi)
    {
        for ($i = 0; $i < count($list_id_jadwal_konsultasi); $i++) {
            $allObat = $this->CI->db->where('id_jadwal_konsultasi', $list_id_jadwal_konsultasi[$i])->get('resep_dokter')->result();
            ...
            foreach ($allObat as $obat) {
                if ($obat->harga != 0) {
                    ...
                    $hargaObat = $this->CI->db->where('id', $obat->id_obat)->get('master_obat')->row()->harga;
