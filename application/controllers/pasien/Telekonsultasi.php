...
class Telekonsultasi extends CI_Controller {
    ...
    public function jadwal(){
        ...
        $data['list_jadwal_konsultasi'] = $this->db->query('SELECT jk.id, jk.tanggal, jk.jam, d.name AS nama_dokter, dr.id AS id_registrasi, dr.id_status_pembayaran, d.foto AS foto_dokter, d.id AS id_dokter, n.poli FROM jadwal_konsultasi AS jk INNER JOIN master_user AS d ON jk.id_dokter = d.id INNER JOIN detail_dokter AS dd ON dd.id_dokter = d.id INNER JOIN data_registrasi AS dr ON dr.id = jk.id_registrasi INNER JOIN nominal AS n ON dd.id_poli = n.id WHERE jk.id_pasien = '.$this->session->userdata('id_user') . ' AND dr.id_status_pembayaran = 1 ORDER BY jk.tanggal, jk.jam ASC')->result();
